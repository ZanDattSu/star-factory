...
**Star Factory** - система обработки заказов, состоящая из **6 микросервисов** с
взаимодействием через **gRPC**, **REST** и **Kafka**, покрывающая полный жизненный цикл заказа:
создание, оплата, асинхронная сборка и уведомление пользователя. Архитектура позволяет
независимо развивать каждый сервис отдельно без влияния на остальные.
...
git clone https://github.com/ZanDattSu/star-factory
cd star-factory
...
#### Собственная обёртка над `uber/zap`
  - Единый формат логов;
...
- Миграции лежат в папке: `{service_name}/migrations/`.
- При старте приложения запускает миграции
- Гарантируется повторяемость структуры БД в любом окружении.

#### Graceful shutdown
Компонент `Closer`, отвечающий за корректное закрытие ресурсов в порядке `LIFO`. Он решает несколько задач:
...
- Конфигурация маппится в go структуры через библиотеку `caarlos0/env` и находятся по пути `{service_name}/internal/config`
- Описана через интерфейсы и структуру-обёртку.
- Можно подменить реализацию конфигов, например на yaml или json
...
**Поднять всю инфраструктуру**: `task up`
...
Посмотреть конфирурацию линтера можно [тут](readme/linter-cfg.md)
...
- HTTP API(chi роутер) строго реализует OpenAPI контракт
...
1. Обработка события `OrderPaid` — уведомление об оплате
   ...
   **Поведение:**
   ...
   - Обращается в AuthService для получения `chat_id` пользователя по `user_uuid`.
