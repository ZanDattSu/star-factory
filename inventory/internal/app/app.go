...
func (a *App) runGRPCServer(ctx context.Context) error {
	logger.Info(ctx, fmt.Sprintf("gRPC InventoryService server listening on %s", config.AppConfig().InventoryGRPC.GRPCAddress()))
...
func (a *App) initHTTPServer(ctx context.Context) error {
	server, err := redirect.NewHTTPServer(ctx,
		config.AppConfig().InventoryGRPC.GRPCAddress(),
		config.AppConfig().InventoryHTTP.HTTPAddress())
	if err != nil {
		return err
	}
...
func (a *App) runHTTPServer(ctx context.Context) error {
	logger.Info(ctx, fmt.Sprintf("HTTP server with gRPC-Gateway listening on %s", config.AppConfig().InventoryHTTP.HTTPAddress()))
